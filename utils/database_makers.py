...
class PlateHole_dataset(Dataset):
    def __init__(self, model_data):
        ...
        if os.path.exists(model_data['path']):
            ...
            ty_profile    = loaded_data['ty_profile'] # (N, nny)
            
            # Build traction2D: (N, NY, NX, 1) — nonzero only at load point
            N, NY, NX = self.material.shape[:3]
            traction2D = np.zeros((N, NY, NX, 1), dtype=np.float32)
            for i in range(N):
            ...
            # for i in range(n):
            #     t1 = default_timer()
            #     self.material[i, :, :, 0], self.disp2D[i, :, :, :] = (
            #         PlateHole_IGA_solver(model_data['beam'], model_data["GRF"]))
            #     print(i, default_timer() - t1)
            np.savez(model_data['path'], material=self.material, disp2D=self.disp2D)
            self.set_data()

    def set_data(self):
        # Input: material + traction2D → 2 channels
        self.x = np.concatenate([self.material, self.traction2D], axis=-1)  # (N, NY, NX, 2)
        self.y = self.disp2D  # (N, NY, NX, 2)
